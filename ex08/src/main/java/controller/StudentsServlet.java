...
public class StudentsServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		response.setContentType("text/html;charset=UTF-8"); //셀렉트문말고는 포스트로보내는게 좋음
		...
		case "/stu/read":
			...
			break;	
		case "/stu/update":
			String scode = request.getParameter("scode");//read랑 거의 비슷함! 
			request.setAttribute("stu", dao.read(scode)); //위에 두줄이 업데이트 할떄 인서트문 데이터넣기위해 리드값을 불러오는거
			request.setAttribute("pageName", "/stu/update.jsp");
			dis.forward(request, response); //디스에 홈이있으니까 홈으로 가라...그 홈에 페이지네임에 업데이트jsp가잇다,,,
			break;
		}
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		case "/stu/delete":
			...
			out.print(dao.delete(scode));//ajax로 호출했을떄 받을값 
			//위에 인서트는 썸밋으로 왓으니 샌드리다이렉트한거고 이건 아니라서 안함
			break;
		case "/stu/update":
			vo= new StuVO();
			vo.setScode(request.getParameter("scode"));
